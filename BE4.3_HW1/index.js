...
app.post("/restaurants", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to add restaurant", details: error.message });
  }
});
...
async function deleteRestaurant(restoId) {
  try {
    const deleteResto = await Restaurant.findByIdAndDelete(restoId);
    return deleteResto;
  } catch (error) {
    console.log(error);
  }
...
app.delete("/restaurants/:restaurantId", async (req, res) => {
  try {
    const deletedResto = await deleteRestaurant(req.params.restaurantId);
    res
      .status(200)
      .json({
        message: "Restaurant deleted successfully",
        restaurant: deletedResto,
      });
  } catch (error) {
